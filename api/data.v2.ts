/**
 * /api/data — DAL-backed rewrite.
 *
 * Differences vs the legacy `data.ts`:
 *
 *   - Reads only what's needed. A `GET /api/data?path=/services/abc` no
 *     longer fetches the entire root state; it goes straight to
 *     `db.services.getById(...)`.
 *
 *   - Writes are atomic. Multi-row replacements call RPC functions that
 *     run inside a single Postgres transaction.
 *
 *   - Stats increments use `db.stats.increment(...)` (RPC), eliminating
 *     the read-modify-write race in the previous implementation.
 *
 *   - Authorization preserved 1:1 from `cloud.ts` so this can be swapped
 *     in without behavioural change.
 *
 * Path handling kept identical to the legacy API:
 *   /                                 (dev only)
 *   /users/:uid[/<field>]
